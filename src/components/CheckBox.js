...
class CheckBox extends Component {
  ...
  onChange = () => {
    const {toggle, ingredient, value} = this.props;
    const addToCart = {name: ingredient, quantity: value.quantity, unit: value.unit};
    toggle(addToCart);
  };

  render() {
    const { ingredient, isActivated = false } = this.props;
    return (
        <div className="switch_box col-sm-2 col-xs-3">
          <div className="input_wrapper">
            ...
            <CheckBoxesSvgChecked/>
            <CheckBoxesSvgUnchecked/>
          </div>
        </div>
    );
  }
...
const CheckBoxesSvgChecked = () => (
    <svg className="is_checked" xmlns="http://www.w3.org/2000/svg"
...
const CheckBoxesSvgUnchecked = () => (
    <svg className="is_unchecked" xmlns="http://www.w3.org/2000/svg"
